...
│   └── styles/main.css
└── supabase/
    ├── migrations/               # 0001 a 0016, aplicadas em ordem no SQL Editor
...
2. **Rodar as migrações, em ordem**: abra o SQL Editor do projeto e execute cada arquivo de `supabase/migrations/` de `0001_initial_schema.sql` até `0016_session_age_check.sql`, um de cada vez, na ordem numérica. Elas criam as tabelas, RLS, funções, triggers e os dois buckets de Storage (`document-files`, `generated-certificates`) — nenhum passo manual de Storage é necessário além disso. A migração 0002 também popula os dados de referência da Estancorp (empresa, hotéis, tipos de documento) — se for adaptar o projeto para outra empresa, edite ou pule essa migração.
...
5. **Conferir a política de senha e MFA**: em Authentication → Sign In / Providers → Email, confirme mínimo de 8 caracteres com letra maiúscula, minúscula e número (é o padrão que este projeto assume — veja [SECURITY_CHECKLIST.md](SECURITY_CHECKLIST.md) para o racional completo e as limitações do plano Free).
